...
{
  IO_Init(GPIOA,GPIO_Pin_4,GPIO_Mode_Out_PP,GPIO_Speed_50MHz,1);//PA4 片选
	SPI1_Init();		   //初始化SPI
...
{ 
  ...
  u32 i;
	...
	{	
		...
		for(i=0;i<secremain;i++)//校验数据（只检查写入位置的状态）
		{
		...
		if(i<secremain)//需要擦除
		{
			...
			SPI_Flash_Write_NoCheck(SPI_FLASH_BUF,secpos*4096,4069);//写入整个扇区  
...
//擦除一个山区的最少时间:150ms
void SPI_Flash_Erase_Sector(u16 Dst_Addr)   
{   
	  Dst_Addr*=4096;
    SPI_FLASH_Write_Enable();                  //SET WEL 	 
    SPI_Flash_Wait_Busy();   
...
void SPI_FLASH_BufferWrite(u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite)//连续写入按页写（自动擦除）
{
